...
// `grandFont`/`subtotalFont` shrink specific label columns just enough that long words
// like "GRAND TOTAL"/"SUBTOTAL" never wrap — a wrapped cell inflates that one row's height
// and makes the table look uneven next to the single-line rows around it.
function themedRowStyler(
  theme: Theme,
  grandIdx: number,
  subtotalRows: number[] = [],
  opts: { grandFont?: Record<number, number>; subtotalFont?: Record<number, number> } = {},
) {
  return (data: CellHookData) => {
    ...
    if (i === grandIdx) {
      ...
      if (opts.grandFont?.[col] !== undefined) data.cell.styles.fontSize = opts.grandFont[col];
    } else if (subtotalRows.includes(i)) {
      data.cell.styles.fillColor = theme.tint;
      data.cell.styles.textColor = theme.solid;
      data.cell.styles.fontStyle = 'bold';
      if (opts.subtotalFont?.[col] !== undefined) data.cell.styles.fontSize = opts.subtotalFont[col];
    }
  };
}
...
export function exportSummaryReport(students: Student[], academicYear: string, subtitle?: string, themeName: ThemeName = 'sky'): void {
  ...
  for (const yr of YEARS) {
    ...
    body.push([YR_LABEL[yr], 'SUBTOTAL', sRegular, sLtrl, sSnq, sRptr, sTotal]);
...
export function exportCategoryReport(students: Student[], academicYear: string, subtitle?: string, themeName: ThemeName = 'emerald'): void {
  ...
  for (const yr of YEARS) {
    ...
    subtotalRows.push(body.length);
    body.push(catRow(YR_LABEL[yr], 'SUB', sub, sTotal));
    for (const k of CAT_KEYS) grand[k] += sub[k];
    gTotal += sTotal;
  }

  const grandIdx = body.length;
  body.push(catRow('TOTAL', '', grand, gTotal));
...
export function exportGenderCourseYearReport(students: Student[], academicYear: string, subtitle?: string, themeName: ThemeName = 'teal'): void {
  ...
  for (const yr of YEARS) {
    ...
    subtotalRows.push(body.length);
    body.push([YR_LABEL[yr], 'SUBTOTAL', sB, sG, sT]);
    gB += sB; gG += sG; gT += sT;
  }

  const grandIdx = body.length;
  body.push(['TOTAL', '', gB, gG, gT]);
...
export function exportGenderCategoryReport(students: Student[], academicYear: string, themeName: ThemeName = 'rose'): void {
  ...
  const grandIdx = body.length;
  body.push(['TOTAL', '', ...CATS.flatMap((cat) => [grandCats[cat].b, grandCats[cat].g]), gB, gG]);
  ...
  autoTable(docL, {
    ...
    didParseCell: themedRowStyler(theme, grandIdx, subtotalRows, { grandFont: { 0: 8 } }),
  });
