...
const TAB_ICONS: Record<TabId, ReactNode> = {
  ...
  'wp-fee-distribution': (
    <svg {...ICON_PROPS}><rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 21V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v16" /></svg>
  ),
  'fee-reg-1': (
...
// Date line for report PDFs that share the day-pill + From/To filter pattern:
// an explicit range wins over the day pill; an empty pill means "All Dates".
function reportDateLabel(selectedDate: string, dateFrom: string, dateTo: string): string {
  if (dateFrom && dateTo) return `${formatDayLabel(dateFrom)} – ${formatDayLabel(dateTo)}`;
  if (dateFrom) return `From ${formatDayLabel(dateFrom)}`;
  if (dateTo)   return `Up to ${formatDayLabel(dateTo)}`;
  return selectedDate ? formatDayLabel(selectedDate) : 'All Dates';
}
...
const PDF_NEAR_BLACK: [number, number, number] = [25, 25, 25];
const PDF_GRID: [number, number, number] = [210, 215, 220];
const PDF_PAD_H = 6; // left+right cellPadding of 3mm each

function pdfDateStr(): string {
  const d = new Date();
  return `${String(d.getDate()).padStart(2, '0')}-${d.toLocaleString('en-US', { month: 'short' })}-${String(d.getFullYear()).slice(2)}`;
}

/** Measures every column except `flexibleIdx` from header/body/footer content, giving the
 *  leftover page width to the flexible column (with a minimum floor), scaling fixed columns
 *  down proportionally if needed so the table never exceeds `usableWidth`. */
function computeAutoWidths(
  ...
  const widths = headers.map((h, idx) => {
    if (idx === flexibleIdx) return 0;
    measureDoc.setFont('helvetica', 'bold');
    let w = measureDoc.getTextWidth(h);
    measureDoc.setFont('helvetica', 'normal');
    for (const row of rows) {
      const cw = measureDoc.getTextWidth(String(row[idx] ?? ''));
      if (cw > w) w = cw;
    }
    const colW = w + PDF_PAD_H + 1.5;
    fixedTotal += colW;
    return colW;
  });

  const availableForFixed = usableWidth - minFlexible;
  if (fixedTotal > availableForFixed) {
    const scale = availableForFixed / fixedTotal;
    for (let i = 0; i < widths.length; i++) if (i !== flexibleIdx) widths[i] *= scale;
    fixedTotal = availableForFixed;
  }
  widths[flexibleIdx] = usableWidth - fixedTotal;
  return widths;
}
...
function pdfHeader(doc: jsPDF, title: string, chips: string, pageW: number, margin: number): number {
  ...
  doc.text(chips, pageW / 2, 19, { align: 'center' });
  doc.setFontSize(7); doc.setTextColor(150, 150, 150);
  doc.text(`Generated: ${pdfDateStr()}`, pageW / 2, 24, { align: 'center' });
  doc.setDrawColor(...PDF_GRID); doc.setLineWidth(0.2);
  doc.line(margin, 26.5, pageW - margin, 26.5);
  doc.setTextColor(...PDF_NEAR_BLACK);
  return 31;
}
...
function pdfFooter(doc: jsPDF, footerLabel: string, margin: number): void {
  ...
  for (let p = 1; p <= pages; p++) {
    ...
    const W = doc.internal.pageSize.getWidth();
    doc.setFont('helvetica', 'normal'); doc.setFontSize(6.5); doc.setTextColor(160, 160, 160);
    doc.text(footerLabel, margin, H - 5);
    doc.text(`Page ${p} of ${pages}`, W - margin, H - 5, { align: 'right' });
  }
}
...
function exportRemittanceDistPdf(dist: FeeDistRow[], label: string, academicYear: string): void {
  ...
  const headers = ['Sl', 'Fee Type', 'Students', 'Fee Amt (₹)', 'Total Allotted', 'To Govt.', 'To SVK', 'To SMP'];
  const FEE_TYPE_IDX = 1;
  ...
  const body: (string | number)[][] = dist.map((r) => [
    r.slNo, r.feeType, r.studentCount, numPdf(r.feeAmountPerStudent), numPdf(r.totalCollected),
    r.toGov > 0 ? numPdf(r.toGov) : '—', r.toSVK > 0 ? numPdf(r.toSVK) : '—', r.toSMP > 0 ? numPdf(r.toSMP) : '—',
  ]);
  const footRow: (string | number)[] = ['', 'GRAND TOTAL', '', '', numPdf(totals.tot), numPdf(totals.gov), numPdf(totals.svk), numPdf(totals.smp)];

  const widths = computeAutoWidths(8.5, headers, [...body, footRow], FEE_TYPE_IDX, usableW, 40);

  const afterHeader = pdfHeader(doc, 'Fee Remittance Abstract', `${label}  ·  AY ${academicYear}`, pageW, margin);

  autoTable(doc, {
    startY: afterHeader, margin: { left: margin, right: margin },
    head: [headers],
    body: dist.length > 0 ? [...body, footRow] : [['', 'No data for the selected filters.', '', '', '', '', '', '']],
    headStyles: { fillColor: PDF_HEAD, textColor: PDF_WHITE, fontStyle: 'bold', fontSize: 8.5, cellPadding: { top: 2.5, right: 3, bottom: 2.5, left: 3 }, lineWidth: 0 },
    bodyStyles: { fontSize: 8.5, cellPadding: { top: 2.5, right: 3, bottom: 2.5, left: 3 }, fillColor: PDF_WHITE, textColor: PDF_NEAR_BLACK, lineColor: PDF_GRID, lineWidth: 0.18 },
    alternateRowStyles: { fillColor: [248, 250, 252] },
    columnStyles: Object.fromEntries(widths.map((w, i) => [i, { cellWidth: w, halign: i === 0 ? 'center' : i === FEE_TYPE_IDX ? 'left' : i === 2 ? 'center' : 'right' }])),
    didParseCell(data) {
  ...
  pdfFooter(doc, `Fee Remittance Abstract — ${label} ${academicYear}`, margin);
  doc.save(`Fee_Remittance_Abstract_${label.replace(/[^a-zA-Z0-9]/g, '_')}_${academicYear}.pdf`);
}
...
// PDF/Excel register of the filtered rows, laid out like the Blue Register:
// Sl, Date, Year, Course, Name, Adm Type, one column per distinct additional
// head (Youth Red Cross, …) and Total.

interface AddlRegCol { key: string; label: string }

interface AddlRegColDef {
  header: string;
  halign: 'left' | 'center' | 'right';
  get: (r: FeeRecord, i: number) => string | number;
}

function addlHeadAmounts(r: FeeRecord): Record<string, number> {
  const out: Record<string, number> = {};
  for (const h of r.additionalPaid) {
    const key = h.label.trim().toUpperCase();
    if (!key || h.amount <= 0) continue;
    out[key] = (out[key] ?? 0) + h.amount;
  }
  return out;
}

function addlTotal(r: FeeRecord): number {
  return r.additionalPaid.reduce((s, h) => s + (h.amount > 0 ? h.amount : 0), 0);
}

function addlRegDate(date: string): string {
  const MONTHS = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  const [ry, rm, rd] = date.slice(0, 10).split('-');
  return `${rd} ${MONTHS[parseInt(rm) - 1]} ${ry}`;
}
...
function exportAdditionalFeeRegisterExcel(
  ...
  const dataRows = rows.map((r, i) => {
    ...
    return [
      i + 1, addlRegDate(r.date), r.year, r.course, nameOf(r), r.admType,
      ...cols.map((c) => amts[c.key] || null),
      addlTotal(r),
    ];
  });
  const colTotals = cols.map((c) => rows.reduce((s, r) => s + (addlHeadAmounts(r)[c.key] || 0), 0));
  const grandTotal = rows.reduce((s, r) => s + addlTotal(r), 0);
  const totRow = ['TOTAL', '', '', '', '', '', ...colTotals.map((v) => v || null), grandTotal];
  const ws = XLSX.utils.aoa_to_sheet([header, ...dataRows, totRow]);
  // Column widths sized so the sheet fits an A4 portrait page when printed
  // (Excel's default orientation) — Name gets the room, the rest stay narrow.
  ws['!cols'] = [
    { wch: 5 }, { wch: 12 }, { wch: 10 }, { wch: 7 }, { wch: 34 }, { wch: 10 },
    ...cols.map((c) => ({ wch: Math.max(9, c.label.length + 1) })),
    { wch: 9 },
  ];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Additional Fee Register');
...
function exportAdditionalFeeRegisterPdf(
  ...
  const fmtN = (v: number) => v > 0 ? v.toLocaleString('en-IN') : '—';
  const amtsByRow = rows.map(addlHeadAmounts);

  const COLUMNS: AddlRegColDef[] = [
    { header: 'Sl',       halign: 'center', get: (_r, i) => i + 1 },
    { header: 'Date',     halign: 'left',   get: (r) => addlRegDate(r.date) },
    { header: 'Year',     halign: 'center', get: (r) => r.year },
    { header: 'Course',   halign: 'center', get: (r) => r.course },
    { header: 'Name',     halign: 'left',   get: (r) => nameOf(r) },
    { header: 'Adm Type', halign: 'center', get: (r) => r.admType },
    ...cols.map((c): AddlRegColDef => ({ header: c.label, halign: 'right', get: (_r, i) => fmtN(amtsByRow[i][c.key] || 0) })),
    { header: 'Total',    halign: 'right',  get: (r) => addlTotal(r).toLocaleString('en-IN') },
  ];

  const colTotals  = cols.map((c) => amtsByRow.reduce((s, a) => s + (a[c.key] || 0), 0));
  const grandTotal = rows.reduce((s, r) => s + addlTotal(r), 0);
  const totalRow = [
    'TOTAL', '', '', '', `${rows.length} record${rows.length !== 1 ? 's' : ''}`, '',
    ...colTotals.map(fmtN),
  ...
  const colWidths: number[] = COLUMNS.map((col, idx) => {
    ...
    measureDoc.setFont('helvetica', 'bold');
    let w = Math.max(measureDoc.getTextWidth(col.header), measureDoc.getTextWidth(String(totalRow[idx])));
    measureDoc.setFont('helvetica', 'normal');
    for (let i = 0; i < rows.length; i++) {
      const cw = measureDoc.getTextWidth(String(col.get(rows[i], i)));
      if (cw > w) w = cw;
    }
    const colW = w + PAD_H + 1.5;
    fixedTotal += colW;
    return colW;
  });

  // Same safeguard as the Blue Register: if many additional heads push the
  // fixed columns past the page, scale widths, font and padding together.
  const availableForFixed = PORTRAIT_W - MIN_NAME_W;
  let scale = 1;
  if (fixedTotal > availableForFixed) {
  ...
  colWidths[NAME_IDX] = PORTRAIT_W - fixedTotal;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(15, 23, 42);
  doc.text('SMP Admissions — Additional Fee Register', margin, 13);
  ...
  const bodyRows = rows.map((r, i) => COLUMNS.map((c) => c.get(r, i)));
  const headers  = [COLUMNS.map((c) => ({ content: c.header, styles: { halign: c.halign } }))];
  ...
  doc.save(`Additional_Fee_Register_${academicYear}.pdf`);
}
...
function AdditionalFeeReceiptsTab({
  ...
  // Export columns — one per distinct additional head, scoped to baseRows so the
  // set doesn't flicker while a search is typed (same as the Blue Register).
  const addCols = useMemo((): AddlRegCol[] => {
    const labelByKey = new Map<string, string>();
    for (const r of baseRows) {
      for (const h of r.additionalPaid) {
        const key = h.label.trim().toUpperCase();
        if (key && h.amount > 0 && !labelByKey.has(key)) labelByKey.set(key, h.label.trim());
      }
    }
    return [...labelByKey.entries()].sort((a, b) => a[1].localeCompare(b[1])).map(([key, label]) => ({ key, label }));
  }, [baseRows]);

  const nameOf = (r: FeeRecord) => studentMap.get(r.studentId)?.studentNameSSLC ?? r.studentName;
    ...
    <div className="flex flex-col gap-2 flex-1 min-h-0">
      <FilterPanel
        ...
        right={<>
          ...
          <ExportBar
            onPdf={() => rows.length > 0 && exportAdditionalFeeRegisterPdf(rows, addCols, nameOf, academicYear, reportDateLabel(selectedDate, dateFrom, dateTo))}
            onExcel={() => rows.length > 0 && exportAdditionalFeeRegisterExcel(rows, addCols, nameOf, academicYear)}
          />
          <button
            onClick={handlePrint}
...
function exportBlueRegisterPdf(
  ...
  const COLUMNS: BlueRegColDef[] = [
    ...
    { header: 'Rpt No', halign: 'left',   get: (r) => r.record.receiptNumber || '—' },
    { header: 'Course', halign: 'center', get: (r) => r.record.course },
    { header: 'Year',   halign: 'center', get: (r) => r.record.year },
    { header: 'Name',   halign: 'left',   get: (r) => r.record.studentName },
...
function BlueRegisterTab({
  ...
  const FIXED_PCT = 3 + 7 + 6 + 5 + 6 + 12 + 6; // Sl, Date, Rcpt No, Course, Year, Name, Total
  const varCount  = SMP_FEE_HEADS.length + (showSvk ? 1 : 0) + visibleAddCols.length;
  const varPct    = (100 - FIXED_PCT) / varCount;
  const colWidths = ['3%','7%','6%','5%','6%','12%', ...Array(varCount).fill(`${varPct}%`), '6%'];
      ...
      <FilterPanel
        ...
        right={<>
          ...
          <ExportBar
            ...
            onPdf={() => exportBlueRegisterPdf(rows, additionalCols, showAdditional, showSvk, academicYear, reportDateLabel(selectedDate, dateFrom, dateTo))}
          />
        </>}
        hasActiveFilters={hasActiveFilters}
        onClear={clearFilters}
      ...
      <div className="flex-1 min-h-0 overflow-auto">
        <table className="w-full table-fixed text-[11px] border-collapse">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                <td colSpan={6 + SMP_FEE_HEADS.length + (showSvk ? 1 : 0) + visibleAddCols.length + 1} className="px-4 py-8 text-center text-gray-400 text-xs">No records match the current filters.</td>
              </tr>
            ) : rows.map((r, i) => {
              const student = studentMap.get(r.record.studentId);
              const [ry, rm, rd] = r.record.date.slice(0, 10).split('-');
...
// ── Tab: WP (Working Professional) Fee Distribution ──────────────────────────
// Working Professionals are evening-college admissions that enroll exactly like a
// Lateral-entry admission (direct 2nd Year entry) — see manualCertificate.ts. They are
// NOT tracked as their own Student records, so this tab does not read `students` at all.
// Counts are entered manually per course/year/category, then fed through the same
// `calcDistribution()` used by the Fee Distribution tab (via synthetic admType='LATERAL'
// placeholders) so every fee-head/remittance-split rule stays byte-identical. This tab
// is fully independent — it never touches FeeDistributionTab/BudgetTab/calcDistribution
// or their persisted data.

function emptyWPCounts(): WPStudentCounts {
  const zero = (): WPCourseYearCounts => ({ reg: 0, lat: 0, snq: 0 });
  return Object.fromEntries(COURSES.map(c => [c, { yr1: zero(), yr2: zero(), yr3: zero() }])) as WPStudentCounts;
}

function buildWPSyntheticStudents(counts: WPStudentCounts): Student[] {
  const list: Student[] = [];
  let idx = 0;
  const YEAR_BY_KEY: Record<'yr1' | 'yr2' | 'yr3', Year> = { yr1: '1ST YEAR', yr2: '2ND YEAR', yr3: '3RD YEAR' };
  for (const c of COURSES) {
    (['yr1', 'yr2', 'yr3'] as const).forEach((yrKey) => {
      const bucket = counts[c][yrKey];
      const groups: [number, AdmCat][] = [[bucket.reg, 'GM'], [bucket.lat, 'GM'], [bucket.snq, 'SNQ']];
      for (const [count, admCat] of groups) {
        for (let i = 0; i < count; i++) {
          list.push({
            id: `wp-${c}-${yrKey}-${admCat}-${idx++}`,
            course: c,
            year: YEAR_BY_KEY[yrKey],
            admType: 'LATERAL',
            admCat,
...
function WPFeeDistributionTab({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    (async () => {
      const rec = await getWPFeeDistribution(academicYear as AcademicYear);
      if (cancelled) return;
      if (rec) {
        setCounts({ ...emptyWPCounts(), ...rec.counts });
        setSavedAt(rec.updatedAt);
      } else {
        setCounts(emptyWPCounts());
        setSavedAt(null);
      }
      setDirty(false);
      setLoading(false);
  ...
  function updateCount(course: Course, yrKey: 'yr1' | 'yr2' | 'yr3', field: keyof WPCourseYearCounts, val: string) {
    const n = Math.max(0, parseInt(val, 10) || 0);
    setCounts(prev => ({ ...prev, [course]: { ...prev[course], [yrKey]: { ...prev[course][yrKey], [field]: n } } }));
    setDirty(true);
  }
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await saveWPFeeDistribution(academicYear as AcademicYear, counts);
      setSavedAt(new Date().toISOString());
      setDirty(false);
    } finally {
      setSaving(false);
