...
import { buildPreviewHTML, printStudentProfile } from '../../utils/printProfile';
...
export function PrintProfileModal({ student, onClose }: Props) {
  const [previewHtml, setPreviewHtml] = useState('');

  useEffect(() => {
    setPreviewHtml(buildPreviewHTML(student));
      ...
      >
        ...
        <div className="px-4 py-2 bg-blue-50 border-b border-blue-100 flex items-center gap-2 shrink-0">
          ...
          <span className="text-xs text-blue-700">
            Prints <strong>2 copies</strong> on a single A4 sheet — Student Copy and College Copy separated by a cut line.
          </span>
        </div>
        ...
        <div className="px-5 py-3 border-t border-gray-100 bg-gray-50/60 shrink-0 flex items-center justify-between">
          <span className="text-[11px] text-gray-400">
            Scroll to preview both copies · Verify all details before printing
          </span>
